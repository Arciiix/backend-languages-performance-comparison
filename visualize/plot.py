        ...
        with open(csv_file, 'r') as file:
            ...
            for line in file:
                if line.startswith("#"):
                    ...
                    metadata[key.replace("#", "").strip()] = round(float(value), 2) if value.replace(".", "").isnumeric() else value
                else:
                    csv_data += line
            ...
            for row in reader:
                x.append(int(row['Id']))
                y.append(float(row[field].replace(',', '.')))  # Replace comma with dot for float conversion
